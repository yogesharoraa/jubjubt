...
import { UseTotalEarning } from "../../Appstore/Api/Dashboard/UseTotalEarning";
import useApiPost from "../../Hooks/PostData";

function TotalEarning(): JSX.Element {
  ...
  sessionStorage.setItem("currencyvalues", data?.data?.Records[0]?.currency)
  sessionStorage.setItem("currency_symbol", data?.data?.Records[0]?.currency_symbol)


  const { data: UserData } = UseTotalEarning();

  const totalUsers = UserData?.data?.total_income ?? 0;
  const lastMonthUsers = UserData?.data?.lastMonth_income ?? 0;
