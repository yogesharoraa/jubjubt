import { UsegetUserTotal } from "../../Appstore/Api/Dashboard/UsegetUserTotal";

function TotalUsers(): JSX.Element {
  const { data: UserData } = UsegetUserTotal();

  // Ensure totalUsers is always a number
  const totalUsers =
    typeof UserData?.data?.total_counts === "number"
      ? UserData.data.total_counts
      : 0;
  // Ensure lastMonthUsers is always a number
  const lastMonthUsers =
    typeof UserData?.data?.lastMonth_Count === "number"
      ? UserData.data.lastMonth_Count
      : Array.isArray(UserData?.data?.lastMonth_Count)
      ? UserData.data.lastMonth_Count.length
      : 0;
  ...
  return (
    <div className="border   border-bordercolor cursor-pointer rounded-lg px-4 py-5">
      <div className="flex justify-between py-1">
        <h2 className="font-poppins font-semibold text-base  text-textcolorsecondary">
          Total Users
        </h2>
        <h2
          className={`${
