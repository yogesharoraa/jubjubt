...
function Stripe() {
  ...
  const initialFormData = useMemo(() => ({
    stripe_public_key: appConfig?.stripe_public_key || '',
    stripe_secret_key: appConfig?.stripe_secret_key || '',
    stripe: appConfig?.stripe === true || appConfig?.stripe === 'true' ? 'true' : 'false',
  }), [appConfig]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!Array.from(changedFields.keys()).length) {
    ...
    try {
      ...
      if (response?.data) {
        ...
        toast.success('Stripe config updated successfully.');
      }
    } catch (error) {
  ...
  const handleStatusToggle = useCallback(async () => {
    ...
    const newStatus = formData.stripe === 'true' ? 'false' : 'true';
    ...
    try {
      ...
      if (response?.data) {
        setFormData((prev) => ({ ...prev, stripe: newStatus }));
        dispatch(setAppConfig(response.data));
        toast.success(`Stripe status updated to ${newStatus} successfully!`);
      }
    } catch (error) {
      toast.error('Status update failed. Please try again.');
    }
  }, [formData.stripe, makeRequest, dispatch]);
  ...
  return (
    <div className="p-4">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        {[
          { label: 'Stripe Public Key', key: 'stripe_public_key' },
