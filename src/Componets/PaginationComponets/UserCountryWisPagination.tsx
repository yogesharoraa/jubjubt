...
function UserCountryWisPagination() {
    ...
    const handleSelect = (value: string) => {
        ...
        dispatch(setPaginationCountryWiseUserList({
            ...pagination,
            current_page: 1,
    ...
    return (
         <div className="flex flex-col  md:flex-row  gap-y-4  justify-center   md:justify-between w-[600px]   md:w-full  px-2  md:px-10 py-6  bg-primary">
            {/* Dropdown */}
            <div className="relative flex gap-2 items-center" ref={dropdownRef}>
                ...
                {isDropdownOpen && (
                    <div className="absolute left-0 z-50 mb-2  border  bg-primary border-bordercolor   text-textcolor rounded-lg shadow-lg bottom-full w-max">
                        {options.map((option) => (
                            <p
                                key={option}
...
export default UserCountryWisPagination;
