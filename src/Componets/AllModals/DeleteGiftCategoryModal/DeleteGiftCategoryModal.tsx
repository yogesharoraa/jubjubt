...
function DeleteGiftCategoryModal() {
    const modalData = useAppSelector((state) => state.modals.DeleteGiftCategoryModal);
    const dispatch = useAppDispatch();

    const close = () => {
        dispatch(hideModal("DeleteGiftCategoryModal"));
    };


    const music_id = sessionStorage.getItem("giftcategoryid")
    ...
    const handleUserDeactivate = async () => {
        ...
        formData.append("gift_category_id", music_id || "");
            ...
            <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <DialogPanel className="w-[90%] max-w-md sm:max-w-lg  bg-white dark:bg-darkBg rounded-2xl p-6 shadow-xl">
                    ...
                    <h2 className="text-center text-xl font-normal text-textcolor  font-poppins mb-6">
                        Are you sure you want to delete this Gift Category ?
                    </h2>
...
export default DeleteGiftCategoryModal;
