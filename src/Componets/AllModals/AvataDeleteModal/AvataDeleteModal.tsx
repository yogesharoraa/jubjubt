...
function AvataDeleteModal() {
    const modalData = useAppSelector((state) => state.modals.AvataDeleteModal);
    const dispatch = useAppDispatch();

    const close = () => {
        dispatch(hideModal("AvataDeleteModal"));
    };


    const music_id = sessionStorage.getItem("avatarId")
    ...
    const handleUserDeactivate = async () => {
        ...
        formData.append("avatar_id", music_id || "");
        formData.append("status", "false");

        try {
            const response = await postData("/Admin/update-avatar", formData, "multipart/form-data");
...
export default AvataDeleteModal;
