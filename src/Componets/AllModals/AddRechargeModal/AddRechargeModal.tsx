...
function AddRechargeModal() {
    const modalData = useAppSelector((state) => state.modals.AddRechargeModal);
    ...
    const close = () => {
        dispatch(hideModal("AddRechargeModal"));
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await postData("/admin/add-transaction-plan", formData, "multipart/form-data");
        ...
        <Dialog open={modalData} onClose={close} as="div" className="z-50">
            <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <DialogPanel className="w-[90%] max-w-md sm:max-w-lg bg-primary border border-bordercolor rounded-2xl shadow-xl">
                    <ModalHeader title="Add Recharge Plan" onClose={close} />
