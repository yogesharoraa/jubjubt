...
function Following_Modal() {
    const modalData = useAppSelector((state) => state.modals.Following_Modal);
    const dispatch = useAppDispatch();

    const close = () => {
        dispatch(hideModal("Following_Modal"));
    };
            ...
            <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <DialogPanel className="w-[90%] max-w-md sm:max-w-lg  bg-white dark:bg-darkBg rounded-2xl p-6 shadow-xl">
                    <div className="flex items-center justify-between  bg-primary">
                        <h2 className="text-textcolor font-medium text-xl font-poppins ">Following</h2>
                                ...
                                {data?.data?.Records?.map((follower) => (
                                    <>
                                        <div className="flex items-center border-b border-[#EFEFEF] dark:border-[#1f1f1f] justify-between p-2 rounded-md cursor-pointer light:hover:bg-gray-100">
                                            <div className='flex'>
                                                <img src={follower?.User?.profile_pic || "/assets/default_user.png"} alt={follower?.User?.user_name} className="w-12 h-12 mr-3 rounded-full" />
                                                <div className='flex flex-col py-2 '>
                                                    <p className='font-poppins text-textcolor text-sm text-left font-semibold '> {follower?.User?.full_name}</p>
                                                    <p className="text-[#747474] font-poppins text-xs text-left">{follower?.User?.user_name}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </>
...
export default Following_Modal;
