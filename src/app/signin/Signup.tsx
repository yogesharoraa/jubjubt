...
export default function Signup() {
  ...
  const handleSignup = async () => {
    ...
    try {
      const response: UpdateUserRes = await postData("/users/signup-user", {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="pb-6 text-[20px] font-medium text-dark text-center">
  {signupSuccess ? "Verify Your Email" : "Signup"}
</h3>
{signupSuccess ? (
  <div className="flex flex-col items-center text-center px-6 py-8 gap-4">
    ...
    <p className="text-dark text-sm leading-relaxed">
      ...
      <br />
      Check your inbox to complete signup.
    </p>

    <button
      className="bg-main-green text-primary rounded-xl px-6 py-2 mt-4 text-sm"
      onClick={() => {
        dispatch(hideModal("Signup"));
        resetForm();
